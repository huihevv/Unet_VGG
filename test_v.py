...
class myUnet(object):
    ...
    def train(self):
        ...
        model.load_weights(midname, by_name=True)
        print('load model is')
        print(midname)
        print("got unet")

        print('predict test data')
        imgs_conv_test = model.predict(imgs_test, batch_size=1, verbose=1)
        print(np.shape(imgs_conv_test))
        np.save("data_new/results/data_self/imgs_conv_test.npy", imgs_conv_test)


    def save_img(self):
        print("array to image")
        imgs = np.load('./data_new/results/data_self/imgs_conv_test.npy')
        #print(imgs)
        for i in range(imgs.shape[0]):
            for j in range (imgs.shape[3]):
                ...
                img.save("./data_new/results/conv/case%dconv%d.jpg" % (i,j))
