...
class myUnet(object):
    ...
    def train(self):
        ...
        model = load_model(midname, custom_objects={'jd_loss':jd_loss})
        print('load model is')
        print(midname)
        print("got unet")
        #model = load_model('./data_new/model/weights.100-0.02.hdf5')
        #model_checkpoint = ModelCheckpoint('data_new/model/unetvgg.hdf5', monitor='loss', verbose=1, save_best_only=True)
        model_checkpoint = ModelCheckpoint('./data_new/model/weights.{epoch:02d}-{loss:.2f}.hdf5', monitor='loss',
                                           verbose=1, save_best_only=True, period=40)
        print('Fitting model...')
        ...
        tbCallBack = keras.callbacks.TensorBoard(log_dir='./tmp/log', histogram_freq=0, write_graph=True,
                                                 write_images=True)
        model.fit(imgs_train, imgs_mask_train, batch_size=4, epochs=200, verbose=1, validation_split=0.2, shuffle=True, callbacks=[model_checkpoint, tbCallBack])
